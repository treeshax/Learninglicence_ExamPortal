...
export default function Exam({session,setSession,now,offline,setOffline,finish}:Props){
  ...
  const warningCount=session?.events.filter(e=>e.severity==='warning').length??0;
  const warningsLeft=Math.max(0,5-warningCount);
